...
public class Alarms {
	...
	public static final String ALARM_ALERT_ACTION = "com.bitmastro.playMusicAlarm.ALARM_ALERT";
	...
	public static final String ALARM_DONE_ACTION = "com.bitmastro.playMusicAlarm.ALARM_DONE";
	...
	// ALARM_DONE_ACTION).
	public static final String ALARM_SNOOZE_ACTION = "com.bitmastro.playMusicAlarm.ALARM_SNOOZE";

	// AlarmAlertFullScreen listens for this broadcast intent, so that other
	// applications
	// can dismiss the alarm (after ALARM_ALERT_ACTION and before
	// ALARM_DONE_ACTION).
	public static final String ALARM_DISMISS_ACTION = "com.bitmastro.playMusicAlarm.ALARM_DISMISS";
	...
	private static ContentValues createContentValues(Alarm alarm) {
		...
		values.put(Alarm.Columns.DURATION, alarm.duration);
	...
	private static void clearSnoozeIfNeeded(Context context, long alarmTime) {
		...
		for (String snoozedAlarm : snoozedIds) {
			final long snoozeTime = prefs.getLong(
					getAlarmPrefSnoozeTimeKey(snoozedAlarm), 0);
	...
	public static void disableExpiredAlarms(final Context context) {
	    ...
	    try (Cursor cur = getFilteredAlarmsCursor(context.getContentResolver())) {
		if (cur.moveToFirst()) {
		    do {
	...
	private static boolean hasAlarmBeenSnoozed(final SharedPreferences prefs,
			final int alarmId) {
		final Set<String> snoozedIds = prefs
				.getStringSet(PREF_SNOOZE_IDS, null);

		// Return true if there a valid snoozed alarmId was saved
		return snoozedIds != null
				&& snoozedIds.contains(Integer.toString(alarmId));
	}
	...
	private static boolean updateAlarmTimeForSnooze(
		...
		final long time = prefs
				.getLong(getAlarmPrefSnoozeTimeKey(alarm.id), -1);
