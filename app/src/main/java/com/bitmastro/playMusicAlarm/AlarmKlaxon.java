...
public class AlarmKlaxon extends Service {
	...
	public class VolumeRunnable implements Runnable {
		...
		VolumeRunnable(AudioManager audioManager, Handler handler,
				int duration, boolean vibrate, int volume) {
			...
			mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC, volume
					* iteration / NUM_ITERATIONS, 0);
			for (short i = 0; i < numBands; i++) {
				equalizer.setBandLevel(i, (short) (minLevel - minLevel
						* iteration / NUM_ITERATIONS));
			}
			equalizer.setEnabled(true);
			next_delay = duration * 1000 / NUM_ITERATIONS; // use 100 steps
			...
			if (mPlaying) {
				if (iteration <= NUM_ITERATIONS) {
					...
					mAudioManager.setStreamVolume(AudioManager.STREAM_MUSIC,
							volume * iteration / NUM_ITERATIONS, 0);
					for (short i = 0; i < numBands; i++) {
						equalizer.setBandLevel(i, (short) (minLevel - minLevel
								* iteration / NUM_ITERATIONS));
					}
					iteration++;
					mVolumeHandler.postDelayed(this, next_delay);
				} else {
					if (vibrate) {
	...
	private void play(Alarm alarm) {
		...
		if (origVolume == -1) {
			origVolume = audioManager
					.getStreamVolume(AudioManager.STREAM_MUSIC);
		}
		if (Log.LOGV) {
			Log.v("AlarmKlaxon.play() " + alarm.id);
		}
		...
		try {
					...
					public void run() {
						if (!audioManager.isMusicActive()) {
							...
							in.setComponent(new ComponentName(
									...
									"com.google.android.music.PlaySongsActivity"));
							// in.putExtra("command", "play");
							in.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
							in.addCategory(Intent.CATEGORY_APP_MUSIC);
							PackageManager packageManager = getPackageManager();
							List<ResolveInfo> activities = packageManager
									.queryIntentActivities(in, 0);
							boolean isIntentSafe = activities.size() > 0;
							Log.e("activities " + activities);
							Log.e("itentsafe " + isIntentSafe);
							startActivity(in);
						}
					}
				});
				audioManager.setStreamVolume(AudioManager.STREAM_MUSIC, 0, 0);
				mHandler.postDelayed(new VolumeRunnable(audioManager, mHandler,
						alarm.duration, alarm.vibrate, alarm.volume), 5000);
			...
			try {
				...
				/*
				 * Start the vibrator after everything is ok with the media
				 * player
				 */
				if (alarm.vibrate) {
					mVibrator.vibrate(sVibratePattern, 0);
	...
	private void enableKiller(Alarm alarm) {
		final String autoSnooze = PreferenceManager
				.getDefaultSharedPreferences(this).getString(
						SettingsActivity.KEY_AUTO_SILENCE,
